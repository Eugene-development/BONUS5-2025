...
export async function POST({ request, cookies }) {
	try {
		...
		const response = await fetch(`${API_CONFIG.baseUrl}/api/email/verification-notification`, {
			method: 'POST',
			headers: {
		...
		console.error('Email verification notification proxy error:', error);
		return json({ message: 'Ошибка сервера при отправке письма' }, { status: 500 });
	}
}
